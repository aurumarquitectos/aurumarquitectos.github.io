...
4. Preparar una propuesta coherente: imagen, enlace, título, copy y proyecto deben explicar la misma decisión.
5. No inventar datos. Si falta ubicación, año, cliente, material o resultado, usar `bloqueado` y pedir confirmación.
6. Registrar la propuesta completa en `REVISION` y dejar `decision_sayri = pendiente`.
7. No modificar `PUBLICACIONES` hasta que `decision_sayri = aprobar`.
8. Al aprobar, materializar el contenido, marcar `aprobación = aprobado`, ejecutar la sincronización y validar la compilación.
9. Si se solicita `ajustar`, conservar la trazabilidad, atender comentarios y devolver la propuesta a revisión.
10. Si se decide `no_publicar`, conservar la decisión en `REVISION` y no activarla en la web.
